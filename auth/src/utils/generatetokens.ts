...
export const generateRefreshTokens = async(user: UserDoc) => {
    const { id, email } = user;
    const accessToken = jwt.sign({ userId: id, email: email }, process.env.REFRESH_TOKEN_KEY!, { audience: id, expiresIn: '1y' });
    try {
...
export const decodeRefreshToken = async(token:string) => {
    const { userId, email } = jwt.verify(token, process.env.REFRESH_TOKEN_KEY!) as JwtPayloadProps;
